...
\begin{frame}
    ...
    \centering
    \includegraphics<1>{beamer-fast-slow}
    \includegraphics<2>{beamer-fast-slow-lyso}
\end{frame}

\begin{frame}
    \frametitle{LYSO scintillator}
    Operation of a scintillator known:
    \begin{itemize}
        \item 
            Ionizing radiation loses its energy by exciting scintillator material
        \item
            Excited atoms or molecules go back to ground state via some intermediate states
        \item
    ...
    \end{itemize}
    \pause
    LYSO scintillator is radioactive itself\\
...
\begin{frame}
    \frametitle{Fast-slow-coincidence circuit}
    \centering
    \includegraphics{beamer-fast-slow-pm}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item
            Dynode: Rising slower, energy proportional $\Rightarrow$ slow signal
    \end{itemize}
\end{frame}

\begin{frame}
    \frametitle{Electronic components}
    \only<1,3,5,7>{\vspace{2cm}}
    \includegraphics<1>{beamer-fast-slow-sca}
    \includegraphics<3>{beamer-fast-slow-cfd}
    \includegraphics<5>{beamer-fast-slow-tac}
    \includegraphics<7>{beamer-fast-slow-mca}

    \onslide<2,4,6,8>{\begin{description}
    \item[SCA]
        “Single channel analizer”. Takes analog signal, gives digital pulse if amplitude lies in certain interval (“SCA window”)
    \item[CFD]<4,6,8>
...
\begin{frame}
    ...
    \begin{itemize}
        \item 
            Indium sample placed inside heat conducting aluminum block
        \pause
        \item
