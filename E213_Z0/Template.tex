...
column
in the matrix. Using the other filters, we can construct the whole matrix.
Table~\ref{tab:matrix/} shows the matrix. It can be seen that it is mostly
diagonal, just as we want it to be. The detection efficiency for the electrons
is not very great. This comes from the severe cutting in $\cos(\theta) < 0.5$
which discards a lot of events.

% TODO Add a table with the matrix.
...
the scattering into electrons. Table~\label{tab:cross-sections} and
Figure~\label{fig:cross-sections} clearly show this apparent violation of
lepton universality. This is an overcompensation in our correction, we should
not have computed the matrix elements with the $t$-channel cut in place. We do
not have access to the machines running \textsc{paw} now, so we can just
...
partial cross section for each of the channels. The results are shown in
Table~\ref{tab:cross-sections}. Errors are the combined errors of luminosity,
counts and matrix elements via the bootstrap method.

% TODO Radiation corrections.
