...
\begin{abstract}
    ...
    We classify a \SI{987}{\nano\meter} class 3B diode laser to have threshold
    current \SI{<< threshold >>}{\ampere} differential efficiency \SI{<<
    differential_efficiency >>}{\watt\per\ampere}. and quantum efficiency
    \num{<< quantum_efficiency >>} and then use it to double the frequency
    within a $\mathrm{KNbO_3}$ crystal. Fundamental wave power is measured
    depending on the crystal temperature, harmonic wave power and polarization,
    last two follow a quartic cosine model.


    The Guassian beams are focused to give a waist of \SI{<< waist_mum
    >>}{\micro\meter} and Rayleigh length \SI{<< rayleigh_length_mm
    >>}{\milli\meter}. Normalized length is \num{<< normalized_length >>}.

    The exactness of the doubling is confirmed with a grating
    and a Michelson interferometer with resolutions of \num{1562.5} and \num{<<
    res_michelson >>} respectively.
...
\begin{figure}
    \centering
    \includegraphics{michelson_setup}
    \caption{%
...
To get a better estimate for a possible deviation from the $2:1$ ratio, we
...
multiple of $\piup$. In other terms the difference in path length is a
...
\begin{figure}
    \centering
    \includegraphics{michelson-plot}
    \caption{%
...
We determined the extinction ratio of the screw-on attenuator to be $F = \num{<< extinction >>}$.
The threshold current of our laser diode is
\SI{<< threshold >>}{\ampere}.

The differential efficiency of the laser is \SI{<< differential_efficiency
>>}{\watt\per\ampere}.
The quantum efficiency~$\eta$ is \num{<< quantum_efficiency >>}.

We were able to focus the Gaussian beam to have a waist of \SI{<< waist_mum
>>}{\micro\meter} and Rayleigh length \SI{<< rayleigh_length_mm
>>}{\milli\meter}. The normalized length of the crystal therefore is \num{<<
normalized_length >>}.

The power dependencies of the polarization angles follow a quartic cosine
model.

The relative uncertainty in the grating should be around \num{6.7e-4}.
We determined the lower bound for the resolution of the Michelson
interferometer to $\lambda/\Deltaup \lambda$ to \num{<< res_michelson >>}. The
interferometer is more accurate than the grating, as expected.
...
\begin{appendix}
    ...
    \begin{longtable}[l]{S}
        ...
        \midrule
        \endhead
        %< for row in mirror_position_table >%
        << ' & '.join(row) >> \\
        %< endfor >%
        \bottomrule
    \end{longtable}
