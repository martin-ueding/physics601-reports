...
The $^{57}\mathrm{Co}$ source is located in a shielding led box. The $\gammaup$
rays escape through an opening in the target's direction. The absorber is a
thin $^{57}\mathrm{Fe}$ plate attached to a motor, which allows us to move it
with tunable but constant velocity over a distance of \SI{25.1 +-
0.2}{\milli\meter} unto the source and away from it. Behind the
absorber there is a proportial detector. It's signal is fed into an amplifier
and then into a single channel analyser (\textsc{SCA}) compare to
figure~\ref{fig:electronics}. After that the signals
are counted in two counters (\textsc{n(lr)} and \textsc{n(rl)}) depending on
the moving direction of the absorber.



First we have to find the measurement's window with an oscilloscope to only
count $\SI{14.4}{\kilo\electronvolt}$ incidents and to reduce noise.
