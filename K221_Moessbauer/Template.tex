...
$v_0$. Using the six values that we have, we obtain $v_0 = \SI{<< isomeric
...
factors, it seems we must expect $g' > g$. Therefore option~\ref{option-2} is
the actual phenomenon. Looking at the intensities of the absorption lines,
option~\ref{option-2} seems to be more likely as adjacent pairs have roughly
the same intensity.

\[
    \Delta v(m_j \text{ fixed}) = \SI{<< v_shift_g >>}{\milli\meter\per\second}
    \eqnsep
    \Delta v(m_j' \text{ fixed}) = \SI{<< v_shift_e >>}{\milli\meter\per\second}
\]
...
this was given by $B = \SI{<< B >>}{\tesla}$. Our values should be compared with
...
\[
    ...
    g'_\text{lit} = \num{-0.15531(4)} \,,
\]
taken from \textcite[Fig.~4.8]{Schatz/Nukleare_Festkoerperphysik}.
