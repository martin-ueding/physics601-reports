...
\textsc{mot}. With the help of a photo diode we then adjust the laser locks to
make the \textsc{mot} even larger.  Replacing the photo diode with the power
meter, we see that out \textsc{mot} is very bright, but not very constant in
power.
Therefor we measure a few times to find a mean value (see
Table~\ref{tab:mot_power}). Our \textsc{mot}'s power is hence \SI{<< power_mot
...
\begin{table}
    ...
    \end{tabular}
    \caption{%
        Measured power of the \textsc{mot}.
    }
    \label{tab:mot_power}
\end{table}
