...
Before we adjust the transversal ($x$ and $y$) beams we try to get equal light power to each
axis. So we measure the fiber power again, which is \SI{15.1}{\milli\watt} and
set the $z$-axis power to to \SI{5.0}{\milli\watt} by rotating the $\lambda/2$
plate behind the fiber. Against our expectations we then have no power on the
transversal elevator. We try it the other way around and adjust the $\lambda/2$
plate again until we have \SI{8.8}{\milli\watt} on the elevator and
\SI{4.8}{\milli\watt} on the $z$-axis.
...
we can see a little difference in the brightness which indicates a
working \textsc{mot}. With the help of a photo diode we then adjust the laser
locks to make the \textsc{mot} even larger.

Because we have already installed the photo diode we then take a few shots of
