...
\begin{abstract}
    ...
    We construct a \mot\ with \SI{<< power_mot >>}{\nano\watt} observable
    fluorescence power, \num{<< atom_number >>} atoms trapped and a volume of
    \SI{<< mot_volume_mm3 >>}{\milli\meter\cubed}. The loading time of our trap
    will be determined to be \SI{<< loading_lifetime >>}{\second}. We compute
    the rubidium scattering cross section to be \SI{<< cross_section
    >>}{\meter\squared}.
\end{abstract}
...
Light, being mediated by massless gauge bosons, has a linear dispersion
...
velocities this gives a force depending force:
\[
...
\]
Applying three orthogonal pairs of lasers gives an effective deceleration of
...
\SI{28}{\percent} of ${}^{87}\text{Ru}$. In our \mot{} we use the former. Part
of its hyperfine level scheme is shown in Figure~\ref{fig:rubidium85}. To
...
transition from $F'=4$ to $F=2$. In this state the atom cannot be excited by
the cooling laser again. Therefore it is called \emph{dark state}. To solve
this problem a second laser, the \emph{pumping laser}, excites the atoms in the
dark state to the $F'=3$ state, from where it can decay into the cooling ground
...
wavelengths the reflected beam of 0-th order points in the same direction, but
the angle of all other orders are frequency dependent. One chooses the position
of the grating so that the $-1$-st order of the wanted frequency is coupled
back into the laser, which causes a strong amplification of that frequency. 
...
further frequency stabilization an error signal is fed back to the Piezo. More
about this error signal in the next sections.

\section{Doppler-free spectroscopy}

Since the Doppler broadening of spectral lines at room temperature is orders of
magnitude bigger than the natural line width, lines which lie close together
cannot be resolved. To nevertheless investigate those lines one has to use
Doppler-free spectroscopy.

For this two counterpropagating laser beams of the same frequency are needed, a
...
two level system with ground state $\ket{\mathrm g}$, excited state
$\ket{\mathrm e}$ and energy difference of $\hbar\omega_0$. If the lasers both
have the frequency $\omega_0$ atoms with $v\neq0$ along the laser's direction
will be out of resonance due to the Doppler effect. Those atoms with $v\approx
0$ will be excited by the pump beam and hence the absorption of the probe beam
is very low. If the lasers are detuned to the red from the resonance frequency
the pump beam and the probe beam excite oncoming atoms with respect to their
own propagation direction. Therefor the absorption of the probe beam is high.
The same holds for blue detuned lasers. This leads to a transmission spectrum
as in Figure~\ref{fig:doppler-free-2-level}. If one takes an absorption
...
(\omega_1+\omega_2)/2$ (see Figure~\ref{fig:doppler-free-3-level}).
...
to the piezo which controls the grating's position. On the oscilloscope we
display the signal from the polarization spectroscopy cell and see the
spectrum. That Doppler-free spectrum of the $F=3$ ground state of
${}^{85}\text{Ru}$ is shown in Figure~\ref{fig:doppler-free-cooling}. From
there we choose the right line, zoom in (reduce scanning amplitude) and lock
the laser to a frequency a bit below the chosen transition. 
...
are hit nicely by the beam. After this is the case we modify the bottom
...
After checking the locking of the lasers again we tweak all the mirrors to
...
\mot. With the help of a photo diode we then adjust the laser locks to make the
\mot{} even larger. We use every degree of freedom from the mirrors and
slightly tune it to optimize the \mot\ signal. We repeat this twice for every
little screw such that we should be very close at the maximum in the parameter
space.
...
Now that the \mot\ is there, we want to characterize it. We are interested in
the number of atoms caught, the volume and scattering cross section of the
rubidium. One needs a few quantities in order to obtain the derived quantities.

\subsection{Lens configuration}

We need the diameter of the lens and its distance to the \mot. Since the
lens is an \SI{1}{\inch} lens, its radius is $d/2 = \SI{<< lens_radius_cm
>>}{\centi\meter}$. Its distance to the \mot\ is $l = \SI{<< lens_distance_cm
>>}{\centi\meter}$, roughly estimated. Then we can work out the solid angle
that is taken up by the lens. The solid angle is $4 \piup$ for the whole
sphere. Therefore we need to take the ratio of the lens surface ($\piup d^2/4$)
and divide it by the total surface of a sphere at distance $l$, $4 \piup l^2$.
In total we will then obtain
\[
...
We then have to find the fluorescence power of the \mot. After replacing the
photo diode with the powermeter, we see that our \mot{} is very bright but
not very constant in power. Therefore we measure a few times to find a mean
value (see Table~\ref{tab:mot_power}). Our \mot's power is hence $p = \SI{<<
power_mot >>}{\nano\watt}$ which is quite good, as the instruction states that
about \SI{50}{\nano\watt} are needed.

With the solid angle we can convert the measured power $p$ into the actual
emitted power of the whole \mot, $p_\text{tot}$. We do this by dividing through
the solid angle:
\[
...
\textsc{cdf}\footnote{cumulative density function} fit. From the fit we derive
...
an individual atom as given by \textcite[Equation~(4)]{wieman/inexpensive}
and already used in our theory chapter:
\[
    ...
    \,.
\]
The authors state the natural line width $\Gamma = \SI{6}{\mega\hertz}$ and
$I_\mathrm s = \SI{4.1}{\milli\watt\per\centi\meter\squared}$
\textcite[328]{wieman/inexpensive}.
We also need the wavelength $\lambda = \SI{<< wavelength_nm >>}{\nano\meter}$.
Using that we can compute $\hbar\omega = \SI{<< hbar_omega_nW_MHz
>>}{\nano\watt\per\mega\hertz}$. The funny unit will become handy later on.
...
This way we can subtract the background. The difference between the two \mot\
signals gives the intensity of the \mot\ itself. We plot that signal together
this the rubidium spectrum, that is shown in Figure~\ref{fig:scan-cooling}.
...
curve to obtain the center of each peak. Then using the level shifts given in
...
\begin{figure}
    ...
    \begin{subfigure}[t]{0.48\linewidth}
        ...
        \caption{%
            Conversion of scan time to actual detuning. The errors are so small
            that one cannot see them.
            }
...
Performing a Gaussian fit to the \mot\ response peak, we can get a good
estimate of the central value of that peak. This should be the detuning that we
have used then we optimized the \mot\ for brightness in the previous steps. The
Gaussian fit gives us a central value of $\Delta = \SI{<< detuning
>>}{\mega\hertz}$.
...
\]
Dividing by the scattering rate we arrive at the expression for the atom number
depending only on observables that we have:
\[
...
Now we want an estimate of our \mot's spatial size. For this we first take a
picture of the chamber with the \mot\ and without it. Then we subtract those
images in order to remove the background from the other laser beams. The
subtracted image is shown in Figure~\ref{fig:difference-3-inv}.
...
minimum value. This should give an good estimation of the \mot\ size. Our
algorithm counts \num{<< mot_white_pixels >>} white pixels belonging to the
\mot. Some dark gray pixels contribute a bit, some light gray pixels do not
contribute fully. We hope that on average this works out. For comparison, the
image used has \num{<< mot_total_pixels >>} pixels.
...
measurements. We first started with the maximum current and then go to no
current. After that we measured a few more points in the middle. While doing
this we noticed deviations from the previous measurements. This could be some
mild hysteresis effect. In principle the copper coils should not have a strong
...
as given by \textcite[Equation~(1)]{wieman/inexpensive}:
\[
...
\parencite[320]{wieman/inexpensive}. The density $n$ can be computed using
\[
...
In this experiment we built up a \mot\ by adjusting the mirrors and
$\lambda/2$ plates on the given setup. We were able to achieve a \mot\ power
of around \SI{<< power_mot >>}{\nano\watt} with our configuration. This is much
higher than needed, which is good.

Then by measuring the cooling laser beam intensity and diameter we were able to
compute the beam intensity and the cooling scattering rate. By measuring the
detuning of the cooling laser we were able to estimate the number of atoms in
our \mot\ to be \num{<< atom_number >>}.

The volume of our \mot\ is \SI{<< mot_volume_mm3 >>}{\milli\meter\cubed},
determined from a photo taken with a camera.

The loading behavior has a time constant of $\tau = \SI{<< loading_lifetime
>>}{\second}$. From that we were able to compute the cross section of
scattering with background rubidium gas go be \SI{<< cross_section
>>}{\meter\squared}. This is a sensible result as the diameter of the hydrogen
atom is around \SI{e-10}{\meter}. Rubidium will be somewhat larger. Taking the
square of this diameter will give a ballpark estimation of the cross section as
we assume short-range scattering.

The large errors on our measurements mostly stem on from the uncertainty of the
distance between the \mot\ and the lens which projects the \mot\ onto the power
meter. Also the pressure was not known precisely. All in all we only get one
significant digit for most derived quantities. If there would be a way to get a
better position measurement of the lens, all errors could be reduced
significantly.

It was nice to have an oscilloscope which would write the raw data to the USB
thumbdrive. That way we could extract the loading time from the raw
measurements and did not have to twiddle with an exported image of the
oscillogram.
